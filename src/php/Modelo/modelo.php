  ...
  class Modelo{
    ...
    function altaDatos(){
      if(isset($_POST["btnterminar"]) && !empty($_POST["btnterminar"]))
      {
        ...
        $this -> idFrase = mysqli_insert_id($this -> conexion);
        ...
        {
          $consultaPosicionyPalabra = "INSERT INTO frases_palabras(idFrase, Posicion, Palabra) VALUES ('".$this -> idFrase."', '".$arrayPosicionHuecos[$i]."', '".$arrayPalabrasHuecos[$i]."');";
    ...
    function insertarDatos(){
      ...
      $consultaCogerDatos = "SELECT F.IdFrase, F.Texto, FP.Posicion, FP.Palabra FROM frases F INNER JOIN frases_palabras FP ON F.idFrase = FP.idFrase WHERE F.IdFrase = ".$this -> idFrase.";";
      ...
      //SELECT count(posicion) AS 'Cantidad' FROM frases_palabras FP INNER JOIN frases F ON FP.IdFrase = F.IdFrase WHERE F.idFrase = 1
/*
      echo $texto."<br>";
      print_r($posicion);
	    echo "<br>";
